-- Knockout scoring fairness fix: monotonicity.
--
-- Antes: si un cruce de KO terminaba empatado en los 90' y se definia por penales,
-- acertar el marcador EXACTO de los 90' pero fallar el ganador de la tanda daba 0
-- (ni siquiera los puntos de resultado). Eso violaba la monotonia: una prediccion
-- mas dificil y precisa (marcador exacto) podia valer MENOS que solo acertar el ganador (+3).
--
-- Ahora: el marcador exacto otorga un bonus de 2 incluso si se fallo el ganador de la
-- tanda. Modelo: +3 por el avance + 2 de bonus por marcador exacto = 5 cuando se aciertan
-- ambos; 2 si solo se clava el marcador del empate y se falla el penal. Orden justo: 5 > 3 > 2 > 0.
-- El avance sigue siendo lo que mas pesa.
--
-- Solo redefine la funcion recalculate_scores (identica a 0003 salvo la rama nueva del
-- scoring de KO y un reason auditable). Re-ejecutar con: select recalculate_scores();
...
insert into app_settings (key, value)
values ('scoring_version', '2026-mvp-v2')
on conflict (key) do update set value = excluded.value, updated_at = now();
