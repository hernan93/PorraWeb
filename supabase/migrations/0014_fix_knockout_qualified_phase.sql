-- Knockout "qualified team" phase fix (off-by-one).
--
-- Bug: el bonus `knockout_phase_qualified_team` premiaba acertar que un equipo
-- LLEGA a una fase del bracket, pero cruzaba la prediccion de la MISMA fase
-- (kp.phase = kt.phase). Como knockout_predictions['round_16'] guarda los GANADORES
-- de octavos (quien avanza a cuartos) y no quien LLEGA a octavos, la regla ignoraba
-- la prediccion de la ronda de 32 (que todos llenaron). Efecto: equipos que dieron la
-- sorpresa y clasificaron (p.ej. Canada a octavos) no daban el bonus a los ~15 que los
-- predijeron, mientras favoritos puestos ganando la fase siguiente si lo daban.
--
-- Fix: "llegar a la fase F" = haber sido predicho como ganador de la fase ANTERIOR.
--   octavos (round_16)      <- ganadores de round_32
--   cuartos (quarter_final) <- ganadores de round_16
--   semis  (semi_final)     <- ganadores de quarter_final
--   final                   <- ganadores de semi_final
--   3er puesto              <- perdedores de semis (predicho como ganador de cuartos
--                              pero NO como finalista)
-- Los puntos (4/5/6/7) se asignan segun la fase REAL alcanzada. El resto de la funcion
-- es identico a 0013. Re-ejecutar con: select recalculate_scores();
...
begin
    ...
    from (
        select
            ...
            case
                ...
                -- Marcador exacto de 90' pero ganador de la tanda de penales fallado:
                -- bonus de marcador (2) sin los 3 del avance. Solo se alcanza esta rama
                -- cuando la de arriba (5) no aplico, es decir: empate en 90' + penal fallado.
                when mp.home_goals = mr.home_goals
                    and mp.away_goals = mr.away_goals then 2
            ...
            case
                when mp.home_goals = mr.home_goals
                    ...
                    and mp.predicted_winner_team_id is distinct from mr.winner_team_id
                    then 'knockout_match_exact_score_wrong_shootout'
    ...
    -- knockout_phase_qualified_team (FIX de fase): premia acertar que un equipo LLEGA a
    -- una fase real. "Llegar a F" = predicho como ganador de la fase anterior.
    insert into tmp_score_lines (participant_id, phase, match_id, points, reason)
    select
        s.participant_id,
        'knockouts',
    ...
    from tmp_knockout_phase_teams kt
    join knockout_predictions kp
        on kp.predicted_team_id = kt.team_id
        and kp.phase = case kt.phase
            when 'round_16' then 'round_32'
            when 'quarter_final' then 'round_16'
            when 'semi_final' then 'quarter_final'
            when 'final' then 'semi_final'
        end
    ...
    where s.phase = 'knockouts'
        and s.status in ('submitted', 'locked')
        and p.approval_status = 'approved'
        and kt.phase in ('round_16', 'quarter_final', 'semi_final', 'final');

    -- 3er puesto: los equipos del partido por el 3er puesto son los PERDEDORES de semis.
    -- Se premia (+7) si el usuario predijo ese equipo llegando a semis (ganador de cuartos)
    -- pero NO como finalista (no lo puso como ganador de su semi).
    insert into tmp_score_lines (participant_id, phase, match_id, points, reason)
    select
        s.participant_id,
        'knockouts',
        ...
        'knockout_phase_qualified_team'
    from tmp_knockout_phase_teams kt
    join knockout_predictions kp_qf
        on kp_qf.predicted_team_id = kt.team_id
        and kp_qf.phase = 'quarter_final'
    join submissions s on s.id = kp_qf.submission_id
    join participants p on p.id = s.participant_id
    where kt.phase = 'third_place'
        and s.phase = 'knockouts'
        and s.status in ('submitted', 'locked')
        and p.approval_status = 'approved'
        and not exists (
            select 1 from knockout_predictions kp_sf
            where kp_sf.submission_id = kp_qf.submission_id
                and kp_sf.phase = 'semi_final'
                and kp_sf.predicted_team_id = kt.team_id
        );
...
insert into app_settings (key, value)
values ('scoring_version', '2026-mvp-v3')
on conflict (key) do update set value = excluded.value, updated_at = now();
